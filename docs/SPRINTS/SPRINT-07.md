...
**Sprint Goal**: Implement schema-based multi-tenancy with per-tenant PostgreSQL schema isolation,
subdomain/domain routing middleware, tenant bootstrapping, and cross-tenant query guards.
...
- After this sprint the core security perimeter is in place; all subsequent backend modules build on
  top.
