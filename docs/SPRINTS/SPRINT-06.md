...
**Sprint Goal**: Implement the core authentication backend: JWT tokens, login/logout, session management, MFA (TOTP + backup codes), passkeys (WebAuthn), and OAuth2 social login with Argon2id password hashing via the Rust layer.
...
- All password operations must use `hash_password` / `verify_password` from the Rust layer — no Python-side hashing.
