...
**Sprint Goal**: Implement async bulk CSV/Excel/JSON import and export with validation error
reporting, and the privacy-first analytics module with Plausible/Fathom integration and consent
gating.
...
- US021 all bulk operations must be async via Celery — never block the request thread for large
  datasets.
