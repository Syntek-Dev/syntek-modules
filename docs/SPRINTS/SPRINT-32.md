...
**Sprint Goal**: Implement the session context package with token refresh and idle timeout, and the generated typed GraphQL API client package.
...
- US043 token refresh must be silent — the user must never see a forced logout due to token expiry during active use.
