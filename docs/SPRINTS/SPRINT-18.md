...
**Sprint Goal**: Implement Cloudinary-backed media management with metadata in the database, and
MinIO-backed document storage with presigned URLs and PDF/doc support.
...
- US031 MinIO credentials must come from `SYNTEK_DOCUMENTS` settings; presigned URLs must have short
  TTLs (default 15 minutes).
- Both modules must enforce tenant-scoped access — a tenant cannot access another tenant's media or
  documents.
