...
Security fixes are applied to the **latest released version** of each module only. Older versions
...
If you are unable to use private vulnerability reporting, email:
**security@syntek-studio.com**
...
We follow coordinated disclosure. We will keep you informed throughout the process and credit
you in the security advisory unless you prefer to remain anonymous.
...
- Vulnerabilities in third-party dependencies (report to the upstream maintainer; we will
...
Dependency updates are automated via **GitHub Dependabot** for all four package stacks:
Python (pip), JavaScript (pnpm/npm), Rust (cargo), and GitHub Actions.

Critical and high severity vulnerabilities are patched within **7 days** of a fix being
available upstream. Moderate and low severity within **30 days**.
...
`.claude/SECURITY.md`. That guide covers OWASP mitigations, authentication rules, database
security, and the Rust encryption layer.
